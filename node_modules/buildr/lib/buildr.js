(function() {
  var Buildr, caterpillar, coffee, csslint, cwd, fs, jshint, jsp, less, path, pro, pulverizr, uglify, util, watchTree,
  ...
  Array.prototype.has = Array.prototype.has || function(value2) {
    var value1, _i, _len;
  ...
  Buildr = (function() {
    ...
    function Buildr(options) {
      ...
      this.config = tmp;
      this.config.buildHandler = this.config.buildHandler || function(err) {
        if (err) {
          _this.log('error', err);
          throw err;
        }
      ...
      this.config.rebuildHandler = this.config.rebuildHandler || function(err) {
        if (err) {
          _this.log('error', err);
          throw err;
        }
    ...
    Buildr.prototype.checkConfiguration = function(next) {
      ...
      this.config.outPath = this.config.outPath || this.config.srcPath;
    ...
    Buildr.prototype.cpSrcToOut = function(next, config) {
      ...
      util.rmdir(config.outPath, function(err) {
        if (err) {
          return next(err);
        }
        return fs.mkdir(config.outPath, function(mkdirError) {
          if (mkdirError) {
            return next(mkdirError);
          }
          return fs.readdir(config.srcPath, function(err, files) {
            var copyableFiles, statTasks;
            if (err) {
              return next(err);
            }
            copyableFiles = [];
            statTasks = new util.Group(function(groupError) {
              var copyTasks;
              copyTasks = new util.Group(function(groupError) {
                log('debug', "Copied " + config.srcPath + " to " + config.outPath);
                return next(err);
              });
              copyTasks.total = copyableFiles.length;
              return copyableFiles.forEach(function(file) {
                return fs.readFile(config.srcPath + '/' + file, 'utf8', function(err, data) {
                  return fs.writeFile(config.outPath + '/' + file, data, 'utf8', copyTasks.completer());
                });
              });
            });
            statTasks.total = files.length;
            return files.forEach(function(file) {
              return fs.stat(config.srcPath + '/' + file, function(error, stat) {
                if (stat.isFile()) {
                  copyableFiles.push(file);
                }
                return statTasks.completer()();
              });
            });
          });
        });
      ...
      this.useOrScan(config.stylesOrder, this.config.outPath, function(fileFullPath, fileRelativePath, next) {
        ...
        switch (extension) {
          case '.css':
            ...
            bundledRelativePath = _this.getRelativePath(config.bundleStylePath, fileFullPath);
            return path.exists(fileFullPath, function(exists) {
              if (exists) {
                source += "@import \"" + bundledRelativePath + "\";\n";
                return next(false);
              } else {
                return util.cp(_fileFullPath, fileFullPath, function(err) {
                  if (err) {
                    return next(err);
                  }
                  source += "@import \"" + bundledRelativePath + "\";\n";
    ...
    Buildr.prototype.getRelativePath = function(srcPath, dstPath) {
      var dstPart, dstParts, i, outParts, outPath, srcPart, srcParts, stillCommon, _i, _j, _len, _len1, _ref;
      srcParts = srcPath.split(/[\/\\]/g);
      dstParts = dstPath.split(/[\/\\]/g);
      outParts = [];
      stillCommon = null;
      for (i = _i = 0, _len = srcParts.length; _i < _len; i = ++_i) {
        srcPart = srcParts[i];
        if (stillCommon === null) {
          if (srcPart !== dstParts[i]) {
            stillCommon = i;
          }
        } else {
          outParts.push('..');
      ...
      _ref = dstParts.slice(stillCommon);
      for (i = _j = 0, _len1 = _ref.length; _j < _len1; i = ++_j) {
        dstPart = _ref[i];
        outParts.push(dstPart);
      }
      return outPath = outParts.join('/');
    };
